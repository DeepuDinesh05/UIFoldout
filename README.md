...
This file has a compile-time dependency on `Unity.Netcode.Runtime`. Since UIFoldout ships
without an `.asmdef` (by design, for drag-and-drop / unitypackage installs), it can't
auto-detect whether Netcode for GameObjects is installed. **It is opt-in** behind a manual
scripting define, so projects without Netcode are completely unaffected:
