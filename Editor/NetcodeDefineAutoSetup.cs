...
// NetworkEditorOverride.cs lives in its own assembly (TyroByte.UIFoldout.Netcode.Editor),
// gated behind a defineConstraint on TYROBYTE_NETCODE_GAMEOBJECTS so its reference to
// Unity.Netcode.Runtime never breaks compilation for projects without Netcode installed.
// This script automates flipping that define: it reflects for the Netcode assembly (no
// compile-time reference, so this file itself needs no scripting-define gate and can
// safely live in the always-compiled main Editor assembly) and offers to toggle
