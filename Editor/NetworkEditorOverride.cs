...
// (Unity.Netcode.Runtime). UIFoldout ships with no asmdef (see README —
// intentional, for drag-and-drop / unitypackage distribution), so there is no
// versionDefines mechanism to auto-detect the package's presence. Instead this
// whole file is opt-in behind a manual scripting define so projects that don't
// use Netcode for GameObjects are unaffected:
//
